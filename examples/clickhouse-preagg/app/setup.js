...
// Run: pnpm run setup (after `airlayer build && airlayer pull` in parent dir)
...
if (!existsSync(resolve(cacheSrc, 'manifest.json'))) {
  console.error('Pre-aggregate cache not found. Run from examples/clickhouse-preagg:\n  airlayer build --config config.yml\n  airlayer pull --config config.yml');
  process.exit(1);
}
...
console.log('\nSetup complete. Run: pnpm run dev');
