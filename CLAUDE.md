...
Airlayer anchors at the period END; the reference anchors at the period START and runs to the period end (`trailingStart = periodStart − trailingDays`, `peerCohortSql.ts:200,219-223`), so its window is a lookback *extension* of the period and always contains it. End-anchoring makes the window's length exactly what the declaration says regardless of how long the period is — at the cost that a `window` shorter than the period no longer contains it, which is why the "in the period but not the band window" exclusion below exists at all.
