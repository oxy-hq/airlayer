...
### Current test counts (~1,065 total)

| Category | Count | What |
|----------|-------|------|
| Unit tests | 424 | SQL generation (110), foreign parsers (50), validator (36), response shaping (28), motifs (24), schema models + parsing (36), CLI (29), plus profiling, joins, member-sql, promotions, shift interval math. Includes inline_params escaping, contrib manifest parsing, gsheets init statements, expr-ref join expansion (#55), **promotion closure + validator + hierarchy-aware RCA pruning**. Includes peer cohorts (41 — of which `engine::cohort` 37, `cli::tests` cohort-reference resolution + `default_cohort` fallback 4; the 8 cohort rules in `schema::validator` are counted in its own 36; see the row below for what they cover) |
| Preagg unit tests | 181 | Hashing, rollup resolution, coverage, re-aggregation SQL, all-dialects build/manifest/reagg, filter rendering, ORDER BY, LIKE escaping, library API, definition-fingerprint immunity to `default_cohort`/`analysis`/`band.window` |
...
902 lib tests (`cargo test --lib -- --list`: 900 pass + 2 ignored) = Unit tests (424) + Preagg unit tests (181) + Metric tree ops (289); the Peer cohorts row is a subset of Unit tests, not additional. The ~1,065 total above adds the four integration/contrib rows (72 + 40 + 21 + 30 = 163) on top of the 902 lib tests.
...
**`band.window:` is the band's own window, and a different axis from `per:`.** Omit it and band and metric are both measured over the query period — the behaviour before the field existed, unchanged, down to still compiling exactly one pull. Set it to an interval (`90 days`, `3 months` — the same grammar `shift.by` uses, parsed by `engine::shift::Interval`) and the band alone moves to a lookback window **anchored at the period start**: `[start - window, end]`, both bounds inclusive, calendar arithmetic via `Interval::subtract_from`. The metric and the `require` tuple stay on the query period. Start-anchoring is parity with the reference (whose window spans `window + period`, so no fixed end-anchored value reproduces it across months of different lengths) and it makes the band window always CONTAIN the period — an entity is never banded on a window that excludes part of the performance being judged.

`per:` and `window:` fix different problems and neither substitutes for the other: `per:` stops a trailing TOTAL conflating size with tenure; `window:` stops a SHORT period being used as a size proxy. The reference needs both at once — a one-month reporting period is noisy as a size estimate (a store that had a slow March is not a smaller store) while still being the month the caller asked about. Before the field, the only way to band on a trailing window was to ask for the trailing period, which moves the metric too — a plausible number over the wrong window, with nothing reporting a problem. That was the one silent failure in a design that is otherwise emphatic that refusals are reported.

Airlayer anchors at the period START, as the reference does (`trailingStart = periodStart − trailingDays`, `peerCohortSql.ts:200,219-223`, summed over `d >= trailingStart AND d <= periodEnd` at `:278`), so the window is a lookback *extension* of the period. That is parity, not a preference: the reference's window spans `window + period`, so an end-anchored declaration would need a different value for a 31-day month than for a 28-day one, and no fixed value expresses both. It also makes the band window always CONTAIN the period, so an entity is never banded on a window that excludes part of the performance being judged. The "in the period but not the band window" exclusion below survives the change, but its cause is the two pulls selecting DIFFERENT MEASURES — a band measure on a view with no rows for that entity — not a window shorter than the period.

A windowed band compiles a **second** entity-grain pull at the band's time range, joined per entity key: the period pull takes `[key] + require` and the target measure, the band pull takes `[key]` ALONE (no `require` — the exact-match tuple is a property of the subject read over the comparison period, and selecting it there would fan the band's group-by out) plus `band.measure`/`band.per`. Every guard extends to it — see **Guards** below. An entity present in one window and not the other is *reported* in `excluded` naming which window it was missing from, never banded on the other window instead: substituting the period row for a missing band row would be the silently-wrong-window defect reintroduced one level down. `PeerCohortResult.band_window` names the window actually used (`Some(period)` for a band with no `window:`, `None` for a cohort with no band), for the same reason `.cohort` does.

`min_peers` is a **reporting** predicate. A subject below the floor comes back in `subjects` with `sufficient: false` and its peer count, not filtered away — the failure this closes is a store that "simply did not appear in the list and no screen said why". Omitting `min_peers` defaults it to `1`, not `0` — a subject with zero peers is never marked sufficient by default. Likewise `excluded` carries everything the pull returned that could not be matched, with a reason: a NULL `require` value, an un-normalisable band, an unreadable measure, or a **NULL entity key** — an orphaned fact row the pull's LEFT JOIN keeps, which is not a subject at all. A keyless row is reported under a synthesized `(null) [<require values>]` id, because one pull can carry several of them and a consumer keying `excluded` by `key` would otherwise collapse them into one. The id comes from the pull's own group-by key (`[key] + require`), so two keyless rows necessarily differ in it and the same data yields the same id on every run — `key` is a serialized public field a consumer may persist or diff. The marker is lengthened if any real key starts with it, so a store genuinely keyed `(null)` stays distinguishable. `subjects` and `excluded` are disjoint and together cover the pulled population: a pulled row is in one or the other, never both and never neither.
...
- **Per-pull, per-window guards** — a band `window:` makes two pulls, and each is guarded against **its own** independent count (`count_entities` / `cross_check_pull`). The counts must be per-window because the two populations differ by construction: an entity that traded in the trailing window but not in the reporting period is in one and not the other, so checking both against a single count would refuse exactly that correct case. The band pull's fan-out message blames the key dimension rather than `require`, which it never selects.
- **Truncation cross-check** — the entity-grain pull sets an explicit `UNBOUNDED_QUERY_LIMIT`, then its row count is compared against an independent `COUNT(DISTINCT key)` (the engine-installed `__cohort_total__` measure, via `augment_layer_for_cohort`). Rows whose entity key is NULL are partitioned off BEFORE that comparison and reported in `excluded` — the pull is `FROM <fact> LEFT JOIN <entity>` (the fact view owns the measures, so it wins `pick_base_view`, and a `ManyToOne` hop always compiles to `LEFT`), so an orphaned fact row emits a NULL-key group that `COUNT(DISTINCT key)` never sees; counting it would make one orphan look like a fan-out and refuse the whole cohort. Both directions then refuse: fewer rows means a warehouse-side cap truncated the universe; more rows means the pull is not at entity grain, usually a `require` member that is not entity-scoped and fanned the group-by out. A row count against a distinct-key count can also *net out* — one key duplicated and another missing passes both directions while double-counting the duplicate in every peer set — so the pull's own `HashSet` of keys is checked against its keyed row count first, naming the repeated keys, without consulting the independent total at all.
...
**Induced measures work as cohort targets, but not by their requested name.** `compile_query` rewrites an induced measure (`stores.wage_pct`) to its source (`sales.wage_pct`) before generating SQL and patches the requested name back onto `QueryResult.columns` only — never onto the executor's rows — so cells arrive keyed `sales__wage_pct`. `resolve_cohort` therefore resolves the target (and `band.measure` / `band.per`) through the promotion closure once up front and reads cells, and the `direction:` polarity, by the **source** name; `PeerCohortResult.measure` and every exclusion reason still report the **requested** one. Reading by the requested name returned zero subjects with every entity excluded "null or unreadable", and the polarity lookup failed the same way — `stores` declares no `wage_pct` to carry a `direction:` — so fixing only the cells would have inverted every gap. `require` dimensions need none of this: promotion is measure-only. An induced name reachable from **two** source views is refused, naming them: a cohort declaration has no `through:` to disambiguate with, and a silent pick would answer a different question from the one asked.
...
- `cohort <measure> --time <dim> --period start:end`: compare every instance of an entity against its own peer group over one window. `--cohort entity.cohort_name` picks which declared cohort to use; omit it and the measure's `default_cohort` is used, and the result names whichever was used either way. `--statistic median|p75|best_peer` picks the baseline statistic over the peer group (default `median`), sharing `opportunity`'s direction handling — `p75` means "75% of the way toward better", `best_peer` is the max for a higher-is-better measure and the min for a lower-is-better one. Always executes (requires config.yml). Add `--json` for machine-readable output. When the cohort's band declares a `window:`, the output also names the lookback window the band was measured over — it differs from `--period`, which is what the compared measure was measured over. Output always includes an "Excluded" section, even when empty: every pulled row dropped before matching is reported with a reason, never silently — usually a subject that could not be compared, but also a row whose entity key was NULL, which is no entity at all. See the **Peer cohorts** section for the schema, the guards, and why membership is non-reciprocal.
