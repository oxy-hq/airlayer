...
- **Custom motifs** are defined as `.motif.yml` files in `motifs/`. They are always single-stage and use the `adds` field directly.

### CTE generation

`wrap_with_motif()` in `src/engine/motifs.rs` handles all SQL generation:

1. **Single-stage** (most motifs): `WITH __base AS (<sql>) SELECT b.*, <adds> FROM __base b`
