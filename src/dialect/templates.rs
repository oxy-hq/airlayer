...
impl SqlTemplates {
    ...
    pub fn mysql() -> Self {
        ...
        t.insert(
            "functions/COUNT_DISTINCT".into(),
            "COUNT(DISTINCT {{ args_concat }})".into(),
        );
        Self::new(t)
    }
...
fn base_templates() -> HashMap<String, String> {
    ...
    t.insert(
        "functions/COALESCE".into(),
        "COALESCE({{ args_concat }})".into(),
    );

    // Expressions
    t.insert(
        "expressions/column_reference".into(),
        "{% if table_name %}{{ table_name }}.{% endif %}{{ name }}".into(),
    );
    t.insert(
        "expressions/column_aliased".into(),
        "{{expr}} {{quoted_alias}}".into(),
    );
    t.insert(
        "expressions/cast".into(),
        "CAST({{ expr }} AS {{ data_type }})".into(),
    );
    t.insert(
        "expressions/binary".into(),
        "({{ left }} {{ op }} {{ right }})".into(),
    );
    t.insert(
        "expressions/is_null".into(),
        "({{ expr }} IS {% if negate %}NOT {% endif %}NULL)".into(),
    );

    // Filters
    t.insert("filters/equals".into(), "{{ column }} = {{ value }}".into());
    t.insert(
        "filters/not_equals".into(),
        "{{ column }} <> {{ value }}".into(),
    );
    t.insert(
        "filters/in".into(),
        "{{ column }} IN ({{ values_concat }})".into(),
    );
    t.insert(
        "filters/not_in".into(),
        "{{ column }} NOT IN ({{ values_concat }})".into(),
    );
    t.insert(
        "filters/set_where".into(),
        "{{ column }} IS NOT NULL".into(),
    );
    t.insert(
        "filters/not_set_where".into(),
        "{{ column }} IS NULL".into(),
    );
