...
VERSION=${AIRLAYER_VERSION:-latest}
...
if [ "$VERSION" == "latest" ]; then
	BINARY_URL="https://github.com/$REPO/releases/latest/download/airlayer-$TARGET"
else
	BINARY_URL="https://github.com/$REPO/releases/download/$VERSION/airlayer-$TARGET"
fi

echo "Downloading airlayer for $TARGET..."
curl -fSL "$BINARY_URL" -o "airlayer-$TARGET"

if [ $? -ne 0 ]; then
	echo "Error: Failed to download airlayer from $BINARY_URL"
	exit 1
fi

# Make the binary executable
chmod +x "airlayer-$TARGET"

# Move the binary to the install directory
mv "airlayer-$TARGET" "$INSTALL_DIR/airlayer"

echo "airlayer version $VERSION for $TARGET has been installed successfully!"
